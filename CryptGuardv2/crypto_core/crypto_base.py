...
class BaseCipher(ABC):
    ...
    @classmethod
    def derive_keys(
        cls, pwd: str | SecureBytes, salt: bytes, profile: SecurityProfile
    ) -> tuple[bytes, bytes]:
    ...
    @abstractmethod
    def encode_chunk(
        idx: int, plain: bytes, nonce: bytes, enc_key: bytes, rs_use: bool, header: bytes
    ) -> tuple[int, bytes]:
    ...
    @abstractmethod
    def decode_chunk(
        idx: int, cipher_blob: bytes, nonce: bytes, enc_key: bytes, rs_use: bool, header: bytes
    ) -> tuple[int, bytes]:
    ...
    def encrypt_file(
        ...
        *,
        ...
        encoder = mixin(cls)
        body = encoder.encrypt_flow(
            src_path, size, salt, enc_key, hmac_key, rs_use, progress_cb, chunk_size
        )
        ...
        if streaming:
            meta["chunk"] = True
        encrypt_meta_json(
            enc_path.with_suffix(enc_path.suffix + META_EXT),
            meta,
            SecureBytes(password.encode()),
            expires_at,
        )

        logger.info("%s enc %s (%.1f MiB)", cls.__name__, src_path.name, size / 1_048_576)
        return str(pack_enc_zip(enc_path))

    @classmethod
    def decrypt_file(
        cls: type[BaseCipher],
        enc_path: os.PathLike | str,
        password: str,
        *,
        ...
        meta = decrypt_meta_json(
            src.with_suffix(src.suffix + META_EXT), SecureBytes(password.encode())
        )
        ...
        decoder = mixin(cls)
        plain_data = decoder.decrypt_flow(
            blob, size, salt, enc_key, rs_use, progress_cb, chunk_size
        )

        if len(plain_data) != size:
            raise ValueError(
                "Decrypted size does not match expected - file may be corrupted, tampered, or decrypted with wrong algorithm."
            )
...
class StreamingMixin(_FlowMixin):
    ...
    def encrypt_flow(
        ...
        with (
            src.open("rb", buffering=chunk_size * 4) as fin,
            ...
            while chunk := fin.read(chunk_size):
                ...
                fut = ex.submit(
                    self.cipher_cls.encode_chunk, idx, chunk, nonce, enc_key, rs_use, header
                )
                futures.append(fut)
    ...
    def decrypt_flow(
        ...
        with BytesIO(blob) as fin, concurrent.futures.ThreadPoolExecutor() as ex:
            ...
            while hdr := fin.read(expected_hdr):
                ...
                fut = ex.submit(
                    self.cipher_cls.decode_chunk, idx, cipher_blob, nonce, enc_key, rs_use, header
                )
                futures.append(fut)
                idx += 1
            total_chunks = idx
    ...
    def decrypt_flow(
        ...
        while pos < len(blob):
            ...
            if pos + clen > len(blob):
                raise ValueError("Truncated chunk payload")
            cipher_blob = blob[pos : pos + clen]
            pos += clen
            _, plain = self.cipher_cls.decode_chunk(
                idx, cipher_blob, nonce, enc_key, rs_use, header
            )
            out += plain
            if cb:
                cb(min(len(out), size))
