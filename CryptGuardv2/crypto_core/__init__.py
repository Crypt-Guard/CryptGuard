...
from .config             import ARGON_PARAMS, SecurityProfile, LOG_PATH, enable_process_hardening
...
from .argon_utils import load_calibrated_params
calib = load_calibrated_params()
if calib and isinstance(calib, dict) and 'time_cost' in calib:
    # Aplicar calibração única a todos os perfis (escalar memory_cost por perfil)
    base_tc, base_mem, base_par = calib['time_cost'], calib['memory_cost'], calib['parallelism']
    ARGON_PARAMS = {
        SecurityProfile.FAST:     {'time_cost': max(1, base_tc // 2), 'memory_cost': base_mem // 2, 'parallelism': base_par},
        SecurityProfile.BALANCED: calib,
        SecurityProfile.SECURE:   {'time_cost': base_tc * 2, 'memory_cost': base_mem * 2, 'parallelism': base_par}
    }
