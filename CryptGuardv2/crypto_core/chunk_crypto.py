...
def encrypt_chunk(
    idx: int, chunk: bytes, nonce: bytes, obf, use_rs: bool, parity: int, header_aad: bytes = b""
):
...
def decrypt_chunk(
    idx: int, nonce: bytes, cipher: bytes, obf, use_rs: bool, header_aad: bytes = b""
):
    if use_rs:
    ...
    combined_aad = header_aad + _aad(idx)
    with TimedExposure(obf):
        plain = ChaCha20Poly1305(obf.deobfuscate().to_bytes()).decrypt(nonce, cipher, combined_aad)
    return idx, plain
