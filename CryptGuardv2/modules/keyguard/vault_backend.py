...
import os, json, time, base64, gzip, io, stat, secrets, threading
...
try:
    ...
    class SecureMemory:
        def __init__(self, b: bytes): self._b = bytearray(b)
        def to_bytes(self) -> bytes: return bytes(self._b)
        def clear(self): self._b[:] = b"\x00"*len(self._b)

try:
    from crypto_core.key_obfuscator import KeyObfuscator  # type: ignore
except Exception:
    class KeyObfuscator:
        def __init__(self, sm: SecureMemory): self._sm = sm
        def obfuscate(self): pass
        def deobfuscate(self): pass
        def clear(self): self._sm.clear()
...
class VaultManager:
    ...
    class _AtomicStorage:
        ...
        def save(self, data: bytes) -> None:
            ...
            with open(self.wal, "wb") as fwal:
                fwal.write(data); fwal.flush(); os.fsync(fwal.fileno())
            # 2) tmp
            ...
            if self.path.exists():
                try: os.replace(self.path, self.bak)
                except Exception: pass
            # 4) finalize
            os.replace(tmp, self.path)
            try: os.remove(self.wal)
    ...
    def create(self, master_password: bytes):
        ...
        try:
            ...
            self.entries.clear(); self.entry_order.clear()
            self._save()
            self._opened = True
            logger.info("KeyGuard Vault criado com sucesso em %s", self.path)
        except Exception as e:
            logger.vault_error("create", "KeyGuard", e, {
    ...
    def open(self, master_password: bytes):
        ...
        try:
            ...
            logger.info("KeyGuard Vault aberto com sucesso: %d entradas carregadas", len(self.entries))
            
        except Exception as e:
            # conta falha para rate-limit em casos de senha/decifração
            try:
                if isinstance(e, WrongPassword) or isinstance(e, CorruptVault):
                    self._rate.record_failure("open")
            except Exception:
                pass
            logger.vault_error("open", "KeyGuard", e, {
                "vault_path": str(self.path),
    ...
    def close(self):
        self._opened = False
        if self._pw_secret: self._pw_secret.clear()
    ...
    def add_or_update_entry(self, name: str, password: str, metadata: Optional[Dict]=None):
        enc = base64.b64encode(password.encode("utf-8")).decode("ascii")
        now = time.time()
        if name in self.entries:
            e = self.entries[name]
            e.password_b64 = enc; e.metadata = metadata or {}; e.modified = now
        else:
            e = VaultEntry(name=name, password_b64=enc, metadata=metadata or {}, created=now, modified=now)
            self.entries[name] = e; self.entry_order.append(name)
        self._autosave()

    # compat
    def upsert_entry(self, name: str, password: str, metadata: Optional[Dict]=None):
        return self.add_or_update_entry(name, password, metadata)

    def delete_entry(self, name: str):
        if name in self.entries:
            del self.entries[name]
            if name in self.entry_order: self.entry_order.remove(name)
            self._autosave()
    ...
    def _save(self):
        ...
        try:
            ...
            logger.vault_error("save", "KeyGuard", e, {
                "vault_path": str(self.path),
                "entries_count": len(self.entries),
                "has_password_secret": self._pw_secret is not None,
                "vault_dir_exists": self.path.parent.exists() if hasattr(self.path, 'parent') else False
            })
            raise
    ...
    def _encrypt_json_inline_salt(self, obj: Dict, password: bytes):
        ...
        key = pwhash.argon2id.kdf(secret.SecretBox.KEY_SIZE, password, salt,
    ...
    def _decrypt_json_inline_salt(self, raw: bytes, password: bytes) -> Dict:
        ...
        try:
            ...
            if len(raw) < SALT_SIZE + secret.SecretBox.NONCE_SIZE:
                raise CorruptVault(f"Arquivo muito curto: {len(raw)} bytes, mínimo {SALT_SIZE + secret.SecretBox.NONCE_SIZE}")
            
            salt, rest = raw[:SALT_SIZE], raw[SALT_SIZE:]
            logger.debug("KeyGuard: salt extraído (%d bytes), resto (%d bytes)", len(salt), len(rest))
            ...
            if rest.startswith(HEADER_MAGIC):
                ...
                try:
                    ...
                    logger.debug("KeyGuard: formato versionado v%d, opslimit=%d, memlimit=%d", 
                               meta.get("v", 0), opslimit, memlimit)
                except json.JSONDecodeError:
                    raise CorruptVault("Header versionado com JSON inválido")
            else:
            ...
            logger.debug("KeyGuard: descriptografia bem-sucedida, dados descriptografados (%d bytes)", len(plain))
            ...
            try:
                ...
                try:
                    ...
                    logger.exception_with_context("KeyGuard: falha ao decodificar JSON (direto e gzip)", e2, {
                        "raw_length": len(raw)
                    })
                    raise CorruptVault("Dados corrompidos (decodificação)") from e2

            logger.debug("KeyGuard: JSON parseado com sucesso, %d entradas encontradas", len(result.get("entries", {})))
            
            return result
            
        except json.JSONDecodeError as e:
            logger.exception_with_context("KeyGuard: Erro ao parsear JSON após descriptografia", e, {
                "raw_length": len(raw),
                "password_length": len(password),
                "decrypted_data_sample": plain[:100].decode("utf-8", errors="replace") if 'plain' in locals() else None
            })
            raise CorruptVault(f"JSON inválido após descriptografia: {e}")
        except Exception as e:
            # Se for erro de senha, logar com menos detalhes
            if "verification" in str(e).lower() or "decrypt" in str(e).lower():
                logger.warning("KeyGuard Vault: senha incorreta ou arquivo corrompido - %s", type(e).__name__)
                raise WrongPassword("Senha incorreta ou arquivo corrompido") from e
            else:
                logger.exception_with_context("KeyGuard: Erro inesperado na descriptografia", e, {
                    "raw_length": len(raw),
                    "password_length": len(password),
                    "salt_size": SALT_SIZE,
    ...
    def update_all_passwords(self, generator, *, length: int = 20, charset_key: str = "full") -> int:
