...
def validate_password(pwd: str) -> bool:
    """
    Exige senha com pelo menos 8 caracteres, contendo letras minúsculas, maiúsculas,
    dígitos, caracteres especiais e com pontuação zxcvbn >= 3.
    """
    if len(pwd) < 8:
...
def get_file_hash(file_path):
    """
    Retorna (bytes_digest, hexdigest) do arquivo especificado.
    """
...
def validate_key_file(expected_hash: str) -> bool:
    """
    Solicita o caminho do arquivo-chave e valida o hash SHA-256.
    """
...
def get_combined_password():
    """
    Solicita a senha (com verificação dupla) e, opcionalmente, um arquivo-chave.
    Retorna uma combinação (bytearray) e o hash do arquivo-chave (se utilizado).
    Nota: Em Python, strings são imutáveis; embora se tente sobrescrevê-las, a limpeza completa não é garantida.
    """
    while True:
    ...
    # Tenta limpar as variáveis de senha; porém, devido à imutabilidade das strings, essa limpeza é limitada.
    pwd1 = "0" * len(pwd1)
    pwd2 = "0" * len(pwd2)
...
def get_single_password():
    """
    Solicita somente a senha (sem arquivo-chave).
    """
    while True:
...
def choose_auth_method():
    ...
    [2] Somente Senha (sem arquivo-chave)
    Retorna (autenticação combinada como bytearray, hash do arquivo-chave se houver).
    """
