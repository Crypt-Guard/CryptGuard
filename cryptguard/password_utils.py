...
def validate_password(password: str) -> bool:
    ...
    if score < 3:
        # Mensagem genérica para não dar muitas dicas a um atacante
...
def get_combined_password():
    ...
    # Tentativa de limpar pwd1 e pwd2 em string (não é 100% efetiva em Python)
    pwd1 = "0" * len(pwd1)
    pwd2 = "0" * len(pwd2)

    combined = password_bytes + key_file_bytes

    # Wipe sensitive variables in memory (parcialmente efetivo)
...
def get_single_password():
    ...
    # Limpar strings (não totalmente efetivo em Python)
    pwd1 = "0" * len(pwd1)
    pwd2 = "0" * len(pwd2)
