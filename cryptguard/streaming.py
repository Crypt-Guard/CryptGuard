...
def encrypt_data_streaming(file_path: str, password: bytearray,
    ...
    """
    Criptografa arquivo grande em modo streaming, chunk a chunk.
    chunk_size pode ser definido dinamicamente; se None, cai no valor padrão CHUNK_SIZE.
    """
...
def decrypt_data_streaming(enc_path: str, password: bytearray):
    """
    Decifra dados criptografados em chunks, salvando com a extensão original
    se presente no metadado.
    """
    ...
    try:
        ...
        out_name = (
            ...
            f"{meta_plain.get('original_ext','')}"  # ← extensão original
        )
