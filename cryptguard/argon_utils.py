...
def generate_key_from_password(password: bytearray, salt: bytes, params: dict, extra: bytes = None):
    ...
    Se 'extra' for fornecido, ele é concatenado à senha para derivação, permitindo
    a incorporação de fatores adicionais (ex.: token efêmero) na derivação da chave.
    Converte 'password' e 'salt' em 'bytes' por compatibilidade com Argon2.
    """
...
def get_argon2_parameters_for_encryption():
    """
    Pergunta ao usuário se deseja customizar os parâmetros de Argon2id. Retorna um dict.
    """
