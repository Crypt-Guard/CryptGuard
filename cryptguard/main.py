...
def reencrypt_file():
    ...
    out_name = None
    try:
        # Descriptografa o arquivo conforme o método (streaming ou single-shot)
        if meta_plain.get("streaming", False):
            decrypt_data_streaming(enc_path, old_pwd)
        else:
            decrypt_data_single(enc_path, old_pwd)

        # Localiza o arquivo recém-gerado (descriptografado)
    ...
    # Se o usuário optou por single-shot, mas o arquivo é grande, exibe aviso.
